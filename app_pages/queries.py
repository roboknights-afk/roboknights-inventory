...
# Held open by a flag, same reasoning as the other pages' dialogs: the
# st.rerun() after submitting is a full-app rerun, so the dialog has to be
# re-called each run to stay up, and clearing the flag is what closes it.
if "show_ask_question" not in st.session_state:
    st.session_state.show_ask_question = False
...
def _close_ask_question():
    st.session_state.show_ask_question = False


def _open_ask_question():
    st.session_state.show_ask_question = True


@st.dialog("Ask a question", on_dismiss=_close_ask_question)
def render_ask_question():
    st.caption("Goes privately to the hosts — nobody else sees it.")
    new_question = st.text_area("Your question", key="new_question")
    if st.button("Submit", icon=":material/send:", type="primary", key="confirm_ask_question"):
...
if is_host:
    ...
    st.button(
        "Ask a question", icon=":material/add_box:", type="primary",
        key="open_ask_question", on_click=_open_ask_question,
    )
    if st.session_state.show_ask_question:
        render_ask_question()
